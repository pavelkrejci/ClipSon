            ...
            if ($remoteTimestamp -gt $lastKnownTimestamp) {
                ...
                if ($downloadResult -and $downloadResult.Success) {
                    try {
                        ...
                        # Only delete temp files if debug is NOT enabled
                        if (-not $global:Config.app.debug_enabled) {
                            Remove-Item $tempDownloadFileGz -ErrorAction SilentlyContinue
                            Remove-Item $tempDownloadFileJson -ErrorAction SilentlyContinue
                        } else {
                            Write-DebugMsg "DEBUG enabled: Keeping remote extracted files: $tempDownloadFileGz, $tempDownloadFileJson"
                        }
                    }
                    catch {
                        ...
                        if (-not $global:Config.app.debug_enabled) {
                            ...
                            Write-DebugMsg "DEBUG enabled: Keeping remote extracted files after error: $tempDownloadFileGz, $tempDownloadFileJson"
                        }
                    }
                }
            }
